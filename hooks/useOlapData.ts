...
export const useOlapData = (corporation: string) => {
	...
	const fetchOlapData = async () => {
		...
		try {
			...
			const url = `https://nikitahub-gru-resta-back-c88a.twc1.net/olap/get_olap_sec?start_date=${formattedDate}&end_date=${formattedDate}&report_id=12&corporation=${encodeURIComponent(corporation)}`
